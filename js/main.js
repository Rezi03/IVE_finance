document.addEventListener('DOMContentLoaded', () => {
  const translations = {
    fr: {
      ...
      // Traductions M&A (Laissées ici même si la logique JS est retirée)
      ...
      // Traductions spécifiques à la page News
      ...
      // Mise à jour du message d'erreur pour être plus explicite sur l'API
  ...
  const buildLayout = (lang) => {
    ...
    // Ordre corrigé: Trading avant Actualités
    const navLinks = [
    ...
    // Séparer les liens principaux de la navigation et le lien 'Cours' (bouton)
    ...
    // SVG pour le soleil (Géométrique et droit - Lucide/Feather style)
    ...
    // Correction du Footer pour l'alignement Gauche/Droite
    footer.innerHTML = `
  ...
  // --- Logic for Contact Form (kept in case contact.html uses it) ---
  ...
  if (newsListEl && statusElNews) {
    // CLÉ API Alpha Vantage (8XOTZVQWJRFVBW8U)
    const API_KEY = '8XOTZVQWJRFVBW8U'; 
    const t = (key) => translations[currentLang][key] || key;

    const formatTime = (time) => {
      if (!time) return '';
      // Alpha Vantage time format is YYYYMMDDTHHMMSS
    ...
    // Map pour traduire les options de filtre en valeurs API Alpha Vantage
    const topicMap = {
        ...
        '': 'all_topics' // Le filtre "Tous les secteurs" (value="")
    };
    ...
    const fetchNews = async (topicKey, sort) => {
      ...
      const apiTopic = topicMap[topicKey] || 'all_topics';
      const sortValue = (sort === 'latest') ? 'LATEST' : 'RELEVANCE'; // Alpha Vantage utilise des majuscules
      
      // URL Alpha Vantage : function=NEWS_SENTIMENT
      const url = `https://www.alphavantage.co/query?function=NEWS_SENTIMENT&topics=${apiTopic}&sort=${sortValue}&apikey=${API_KEY}`;
      
      try {
        const response = await fetch(url);
        
        // Tentative de lire le JSON pour vérifier les erreurs d'API (limite)
        const data = await response.json(); 

        // 1. VÉRIFICATION DU MESSAGE D'ERREUR ALPHA VANTAGE (limite)
        if (data["Note"] || data["Error Message"]) {
            statusElNews.textContent = t('newsError') + ' (Limite API ou Clé Invalide)';
        ...
        // Cette erreur est souvent une erreur réseau ou JSON si l'API est down
        console.error('Error fetching news:', error);
        statusElNews.textContent = t('newsError') + ' (Échec de la connexion/réponse)';
        newsListEl.innerHTML = `<p class="status-message error">${t('newsError')}</p>`;
      }
    };
